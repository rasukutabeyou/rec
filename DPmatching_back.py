...
for file_index1 in range(1, 101):
    ...
    # ファイル1の読み込み
        ...
        #局所距離の計算
        local_dist = []
        for i in range(frame_count_1 -1, -1, -1):
            row = []
            for j in range(frame_count_2 - 1, -1, -1):
                dist_sq = 0
                for k in range(demention , -1 , -1):  
        ...
        #最小値の取得
        if resualt_min > cumu_dist[frame_count_1 - 1][frame_count_2 - 1]:
            resualt_min = cumu_dist[frame_count_1 - 1][frame_count_2 - 1]
            count_min = count
    if file_index1 == count_min:
        p += 1
    print(f"{dir2}_{count_min:03}.txt")
    print("////////////////////")
#認識確率
p_resualt = p
